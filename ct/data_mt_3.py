...
def collate_pool(dataset_list):
    ...
    N_max = max(atom_fea.shape[0] for (atom_fea, _), _,_,_, _ in dataset_list)
    ...
    for (atom_fea, atom_coords), target1,target2, target3,cif_id in dataset_list:
    ...
    return (batch_atom_fea, batch_coords, batch_mask), batch_target1,batch_target2,batch_target3, batch_cif_ids
...
class CIFData(Dataset):
    ...
    def __getitem__(self, idx):
        cif_id, target1,target2,target3 = self.id_prop_data[idx]
        try:
        ...
        return (atom_fea, atom_coords), target1,target2,target3, cif_id
