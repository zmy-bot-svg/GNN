...
def collate_pool(dataset_list):
    ...
    N_max = max(atom_fea.shape[0] for (atom_fea, _), _, _ in dataset_list)
    batch_atom_fea, batch_coords, batch_mask = [], [], []
    batch_target = []
    batch_cif_ids = []

    for (atom_fea, atom_coords), target, cif_id in dataset_list:
    ...
    return (batch_atom_fea, batch_coords, batch_mask), batch_target, batch_cif_ids
...
def collate_pool_train(dataset_list):
    ...
    for (atom_fea, atom_coords), target, cif_id in dataset_list:
        ...
        if random.random()<0.5:
            atom_coords = _augment_coordinates(atom_coords)
        n_i = atom_fea.shape[0]  
        pad_atom_fea = np.pad(atom_fea, ((0, N_max - n_i), (0, 0)), mode='constant', constant_values=0)
        pad_atom_coords = np.pad(atom_coords, ((0, N_max - n_i), (0, 0)), mode='constant', constant_values=0)
        mask = np.zeros((N_max, 1), dtype=int)
...
def _augment_coordinates( coords, max_translation=5, max_rotation=np.pi):
        """
...
class CIFData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (atom_fea, atom_coords), target, cif_id
