...
def augment_coordinates(coords, max_translation=1, max_rotation=np.pi):
        """
...
def collate_pool(dataset_list):
    ...
    N_max = max(atom_fea.shape[0] for (atom_fea, _), _,_, _ in dataset_list)
    batch_atom_fea, batch_coords, batch_mask = [], [], []
    batch_target1 = []
    batch_target2 = []
    batch_cif_ids = []

    for (atom_fea, atom_coords), target1,target2,cif_id in dataset_list:
    ...
    return (batch_atom_fea, batch_coords, batch_mask), batch_target1,batch_target2, batch_cif_ids
...
def collate_pool_train(dataset_list):
    ...
    for (atom_fea, atom_coords), target1,target2,cif_id in dataset_list:
        ...
        if random.random() < 0.5:
            atom_coords = augment_coordinates(atom_coords)
...
class CIFData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (atom_fea, atom_coords), target1,target2, cif_id
